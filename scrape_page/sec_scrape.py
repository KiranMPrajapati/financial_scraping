...
def process_batch_and_save(batch_df, batch_num, output_dir, semaphore):
    with semaphore:
        processed_rows = process_batch_threaded(batch_df)
        # year = 2024
        process_chunked_df(processed_rows, output_path=f"{output_dir}/Batch_{batch_num}")
...
def scrape(input_dir, output_dir):
    ...
    companies_df = pd.read_parquet(input_dir)
    # year = 2021
    companies_df = companies_df[companies_df['year'] == year]
    companies_df = companies_df.drop_duplicates(subset=['reportDate', 'primaryDocument', 'accessionNumber'], keep='first')
    ...
    for i in [69, 72, 73, 74]:
        ...
        threading.Thread(target=process_batch_and_save, args=(chunked_df, i, output_dir, semaphore)).start()
